...
import ubik3d.model.LightSource;
...
public class DefaultFurnitureCatalog extends FurnitureCatalog {
  ...
  public enum PropertyKey {
    ...
    ID("id"),
    /**
     * The key for the metadata of a piece of furniture .
     */
    METADATA("metadata"),
    /**
     * The key for the rol of a piece of furniture .
     */
    ROL("rol"),
    /**
  ...
  public DefaultFurnitureCatalog(final UserPreferences preferences, 
    ...
    // Try to load ubik3d.io.DefaultFurnitureCatalog property file from classpath 
    ...
    // Try to load ubik3d.io.ContributedFurnitureCatalog property file from classpath 
    ...
    // Try to load ubik3d.io.AdditionalFurnitureCatalog property file from classpath
  ...
  protected CatalogPieceOfFurniture readPieceOfFurniture(ResourceBundle resource, 
    ...
    String rol = null;
    try {
      rol = resource.getString(PropertyKey.ROL.getKey(index));
    } catch (MissingResourceException ex) {
      // Return null if key rol# doesn't exist
      
      return null;
    }
    
    String metadata = resource.getString(PropertyKey.METADATA.getKey(index));
    float amount = Float.parseFloat(resource.getString(PropertyKey.AMOUNT.getKey(index)));
    ...
    if (doorOrWindow) {
      ...
      return new CatalogDoorOrWindow(id, metadata, rol, name, description, icon, planIcon, model, amount,
      ...
      if (lightSources != null) {
        return new CatalogLight(id, metadata, rol, name, description, icon, planIcon, model, amount,
            width, depth, height, elevation, movable, lightSources, modelRotation, creator, 
            resizable, deformable, texturable, price, valueAddedTaxPercentage);
      } else {
        CatalogPieceOfFurniture cpof =  new CatalogPieceOfFurniture(id, metadata, rol, name, description, icon, planIcon, model, amount,
  ...
  private void add(FurnitureCategory pieceCategory,
    ...
    try {        
      ...
      if (piece instanceof CatalogDoorOrWindow) {
        ...
        piece = new CatalogDoorOrWindow(doorOrWindow.getId(), doorOrWindow.getMetadata(), doorOrWindow.getRol(), suffixedName,
            doorOrWindow.getDescription(), doorOrWindow.getIcon(), doorOrWindow.getPlanIcon(), doorOrWindow.getModel(), doorOrWindow.getAmount(),
        ...
        piece = new CatalogLight(light.getId(), light.getMetadata(), light.getRol(), suffixedName,
            light.getDescription(), light.getIcon(), light.getPlanIcon(), light.getModel(), light.getAmount(),
        ...
        piece = new CatalogPieceOfFurniture(piece.getId(), piece.getMetadata(), piece.getRol(), suffixedName,
            piece.getDescription(), piece.getIcon(), piece.getPlanIcon(), piece.getModel(), piece.getAmount(),

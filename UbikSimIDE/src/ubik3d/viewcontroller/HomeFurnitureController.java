...
public class HomeFurnitureController implements Controller {
  ...
  public enum Property {
    ICON, METADATA, ROL, NAME, NAME_VISIBLE, AMOUNT, X, Y, ELEVATION, ANGLE_IN_DEGREES, BASE_PLAN_ITEM, WIDTH, DEPTH, HEIGHT, PROPORTIONAL, COLOR, PAINT, SHININESS, VISIBLE, MODEL_MIRRORED, LIGHT_POWER, RESIZABLE, DEFORMABLE, TEXTURABLE
  }
  ...
  public TextureChoiceController getTextureController() {
    ...
    if (this.textureController == null && this.contentManager != null) {
      this.textureController = new TextureChoiceController(this.preferences.getLocalizedString(
          HomeFurnitureController.class, "textureTitle"), this.preferences, this.viewFactory, this.contentManager);
  ...
  /**
   * Updates edited properties from selected furniture in the home edited by
   * this controller.
   */
  protected void updateProperties() {
    ...
    if (selectedFurniture.isEmpty()) {
      ...
      String metadata = firstPiece.getMetadata();
      if (metadata != null) {
        for (int i = 1; i < selectedFurniture.size(); i++) {
          if (!metadata.equals(selectedFurniture.get(i).getMetadata())) {
            metadata = null;
            break;
          }
        }
      ...
      String rol = firstPiece.getRol();
      if (rol != null) {
        for (int i = 1; i < selectedFurniture.size(); i++) {
          if (!rol.equals(selectedFurniture.get(i).getRol())) {
            rol = null;
            break;
          }
        }
      ...
      Float amount = firstPiece.getAmount();
      for (int i = 1; i < selectedFurniture.size(); i++) {
        if (amount.intValue() != selectedFurniture.get(i).getAmount()) {
          amount = null;
          break;
        }
      }
      ...
      for (int i = 0; i < selectedFurniture.size(); i++) {
        ...
        if (piece.getColor() != null || piece.getTexture() != null) {
  ...
  public void setRol(String rol) {
    if (rol != this.rol) {
      ...
      this.rol = rol;
      this.propertyChangeSupport.firePropertyChange(Property.ROL.name(), oldRol, rol);
    }
  }
  ...
  /**
   * Sets the edited metadata.
   */
  public void setMetadata(String metadata) {
    if (metadata != this.metadata) {
      String oldMetadata = this.metadata;
      this.metadata = metadata;
      this.propertyChangeSupport.firePropertyChange(Property.METADATA.name(), oldMetadata, metadata);
    }
  }
  ...
  public void setAmount(Float amount) {
    if (amount != this.amount) {
      ...
      this.amount = amount;
      this.propertyChangeSupport.firePropertyChange(Property.AMOUNT.name(), oldAmount, amount);
    }
  }
  ...
  public void setAngleInDegrees(Integer angleInDegrees) {
    if (angleInDegrees != this.angleInDegrees) {
      ...
      this.propertyChangeSupport
          .firePropertyChange(Property.ANGLE_IN_DEGREES.name(), oldAngleInDegrees, angleInDegrees);
    }
  }
  ...
  /**
   * Sets whether the piece shininess is the default one, matt, shiny or
   * unknown.
   */
  ...
  /**
   * Returns whether the piece is shininess is the default one, matt, shiny or
   * unknown.
   */
  ...
  /**
   * Sets whether the color or the texture of the furniture model can be changed
   * or not.
   */
  ...
  /**
   * Returns whether the color or the texture of the furniture model can be
   * changed or not.
   */
  ...
  public void modifyFurniture() {
    ...
    if (!selectedFurniture.isEmpty()) {
      ...
      Float angle = getAngleInDegrees() != null
          ? (float) Math.toRadians(getAngleInDegrees())
          : null;
      ...
      // Create an array of modified furniture with their current properties
      // values
      ...
      // Apply modification
      doModifyFurniture(modifiedFurniture, metadata, rol, name, nameVisible, amount, x, y, elevation, angle, basePlanItem, width, depth,
          height, defaultColorsAndTextures, color, texture, defaultShininess, shininess, visible, modelMirrored,
          lightPower);
      List<Selectable> newSelection = this.home.getSelectedItems();
      if (this.undoSupport != null) {
        UndoableEdit undoableEdit = new FurnitureModificationUndoableEdit(this.home, this.preferences, oldSelection,
            newSelection, modifiedFurniture, metadata, rol, name, nameVisible, amount, x, y, elevation, angle, basePlanItem, width, depth,
            height, defaultColorsAndTextures, color, texture, defaultShininess, shininess, visible, modelMirrored,
            lightPower);
  ...
  /**
   * Undoable edit for furniture modification. This class isn't anonymous to
   * avoid being bound to controller and its view.
   */
  private static class FurnitureModificationUndoableEdit extends AbstractUndoableEdit {
    ...
    private FurnitureModificationUndoableEdit(Home home, UserPreferences preferences, List<Selectable> oldSelection,
                                              ...
                                              ModifiedPieceOfFurniture [] modifiedFurniture, String metadata, String rol, String name,
                                              Boolean nameVisible, Float amount, Float x, Float y, Float elevation, Float angle,
                                              Boolean basePlanItem, Float width, Float depth, Float height,
                                              boolean defaultColorsAndTextures, Integer color, HomeTexture texture,
                                              boolean defaultShininess, Float shininess, Boolean visible,
                                              Boolean modelMirrored, Float lightPower) {
      this.home = home;
      this.preferences = preferences;
    ...
    public void redo() throws CannotRedoException {
      super.redo();
      doModifyFurniture(this.modifiedFurniture, this.metadata, this.rol, this.name, this.nameVisible, this.amount,
          this.x, this.y, this.elevation,
          this.angle, this.basePlanItem, this.width, this.depth, this.height, this.defaultColorsAndTextures,
          this.color, this.texture, this.defaultShininess, this.shininess, this.visible, this.modelMirrored,
          this.lightPower);
  ...
  private static void doModifyFurniture(ModifiedPieceOfFurniture [] modifiedFurniture, 
                                        String metadata, String rol, String name, Boolean nameVisible, 
                                        Float amount, Float x, Float y, Float elevation, Float angle,
                                        Boolean basePlanItem, Float width, Float depth, Float height,
                                        boolean defaultColorsAndTextures, Integer color, HomeTexture texture,
                                        boolean defaultShininess, Float shininess, Boolean visible,
                                        Boolean modelMirrored, Float lightPower) {
    for (ModifiedPieceOfFurniture modifiedPiece : modifiedFurniture) {
      ...
      piece.setMetadata(metadata != null
          ? metadata : piece.getMetadata());
      piece.setRol(rol != null
          ? rol : piece.getRol());
      piece.setAmount(amount != null
          ? amount.floatValue() : piece.getAmount());
      if (name != null) {
        piece.setName(name);
      }
      if (nameVisible != null) {
  ...
  /**
   * Restores furniture properties from the values stored in
   * <code>modifiedFurniture</code>.
   */
  ...
  private static class ModifiedPieceOfFurniture {
    ...
    public ModifiedPieceOfFurniture(HomePieceOfFurniture piece) {
      this.piece = piece;
      this.metadata = piece.getMetadata();
      this.rol = piece.getRol();
      this.name = piece.getName();
      this.nameVisible = piece.isNameVisible();
      this.amount = piece.getAmount();
      this.x = piece.getX();
      this.y = piece.getY();
    ...
    public void reset() {
      this.piece.setMetadata(this.metadata);
      this.piece.setRol(this.rol);
      this.piece.setName(this.name);
      this.piece.setNameVisible(this.nameVisible);
      this.piece.setAmount(amount);
    ...
    public void reset() {
      ...
      for (int i = 0; i < groupFurniture.size(); i++) {
        ...
        if (group.isResizable()) {
          // Restore group furniture location and size because resizing a group
          // isn't reversible

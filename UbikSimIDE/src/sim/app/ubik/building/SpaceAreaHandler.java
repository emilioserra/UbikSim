...
public class SpaceAreaHandler extends ElementsHandler {
     ...
     * Comprueba en qué habitaciones se encuentran los puntos de acceso y
     * añade la puerta a dichas habitaciones.
     * @param home
     */
    ...
    private void generateCorridorsAndRooms(Home home) {
        System.out.println("Generando Pasillos y Habitaciones:");
        for (ubik3d.model.Room room : home.getRooms()) {
            if (rols.contains(room.getType())) {
                Room r = createRoom(floor, room);
                if (r != null) {
                    System.out.println("Habitanción creada: " + r.getName());
    ...
    public boolean isWall(int x, int y) {
        if (grid.getObjectsAtLocation(x, y).size() != 1) {
            return false;
        } else {
            Cell cell = (Cell) grid.getObjectsAtLocation(x, y).get(0);
 ...
 */
    public boolean isObstacle(int x, int y, int height) {
        ...
        if (cell != null) {
            if (cell.getValue() instanceof Wall) {
                ...
                if(w.getHeight()> height ) return true;
                return false;
            }
    ...
    /**
     * Returns a list of SpaceArea which contain the SpaceArea whose name contains the specified string (name).
     * 
